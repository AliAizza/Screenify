...
export default function List({ searched }) {
  ...
  if (page === "/") {
    ...
    data_to_display = data.filter(
      (display) => display.category === "TV Series"
    );
    title = "TV Series";
  } else if (page === "/Bookmarked") {
    data_to_display = data.filter((display) => display.isBookmarked === true);
    bookmarked_series = data_to_display.filter(
      (display) => display.category === "TV Series"
    );
    data_to_display = data_to_display.filter(
      (display) => display.category === "Movie"
    );
    title = "Bookmarked Movies";
  }
  if (searched !== "") {
    if (page === "/")
      data_to_display = data.filter((display) =>
        display.title
          .toLowerCase()
          .replace(/\s/g, "")
          .includes(searched.toLowerCase().replace(/\s/g, ""))
      );
    else
      data_to_display = data_to_display.filter((display) =>
        display.title
          .toLowerCase()
          .replace(/\s/g, "")
          .includes(searched.toLowerCase().replace(/\s/g, ""))
    ...
    title = `Found ${
      data_to_display.length > 0 ? data_to_display.length : ""
    } ${
      data_to_display.length === 0
        ? "nothing like"
        : data_to_display.length === 1
        ? "result for"
        : "results for"
    } ‘${searched}’`;
  }
  return (
    <div
      className={page === "/" && searched === "" ? "list" : "list top-space"}
    >
      <h1>{title}</h1>
        ...
        {data_to_display.map((displayed, index) => (
          <div className="displayed-container" key={index}>
            <div className="displayed-hover">
              <Image
                className="displayed-thumbnail"
                src={displayed.thumbnail.regular.large}
                alt="thumbnail"
                width={280}
                height={174}
              ></Image>
            ...
            <div className="displayed-infos">
              ...
              <div className="category">
                <Image
                  src={displayed.category === "Movie" ? mv : tv}
                  alt="tv-mv icon"
                ></Image>
            ...
            <Image
              className="bookmark"
              src={displayed.isBookmarked === true ? bookfull : bookempty}
              alt="bookmark"
            ></Image>
          </div>
        ))}
              ...
              <div className="displayed-container" key={index}>
                <div className="displayed-hover">
                  <Image
                    ...
                    src={
                      width > 1060
                        ? displayed.thumbnail.regular.large
                        : width > 760
                        ? displayed.thumbnail.regular.medium
                        : displayed.thumbnail.regular.small
                    }
                    alt="thumbnail"
                    width={width > 1060 ? 280 : width > 760 ? 220 : 164}
                    height={width > 1060 ? 174 : width > 760 ? 140 : 110}
                  ></Image>
                  <div className="play-hover">
                    <Image src={play} alt="play icon"></Image>
                    <h4>Play</h4>
